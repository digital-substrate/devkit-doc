...
Dual Reality
   The architectural pattern behind dsviper's static side. Two coexisting
   "realities" — *Developer Reality* (typed, static, IDE-friendly: STL in
   C++, type hints in Python) and *Runtime Reality* (dynamic,
   metadata-driven, polymorphic) — bridged by code generated by
   `kibo-template-viper`. The generated code is a set of
   **adapters** over Viper's dynamic API : no parallel implementation,
   no divergence between languages, no drift with the
   {term}`static and dynamic APIs <static API / dynamic API>`. The
   runtime is dynamic by nature ; the static side is generated for the
   comfort of developers and IDEs.
