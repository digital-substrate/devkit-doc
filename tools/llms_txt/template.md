...
- [Bundle coordinates and download URLs](https://devkit.digitalsubstrate.io/version.json): Machine-readable JSON on the sister site — current LTS, build number, every runtime binding with the version it ships and its floor (dsviper on PyPI, @digitalsubstrate/dsviper on npm), and direct download URLs for the DevKit zip / doc archive / PDF / NotebookLM zip.
